...
const EXPECTED_TOKEN = "only4premium";
...
export default async function handler(req, res) {
  try {
    ...
    if (!number) return jsonExit(res, { status: "error", message: "Number is required" }, 400);
    if (!token || !tokensEqual(EXPECTED_TOKEN, token))
      return jsonExit(res, { status: "error", message: "Token expired or invalid" }, 401);
